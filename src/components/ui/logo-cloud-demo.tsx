        ...
        <div className="mx-auto mt-10 grid h-fit w-full grid-cols-1 gap-4 px-4 md:grid-cols-2 lg:grid-cols-3 lg:gap-6">
          {partnerLogos.map((partner) => (
            <div
              key={partner.id}
              className={cn(
                "group relative aspect-video max-h-[120px] w-full cursor-default overflow-hidden rounded-[0.35rem] border bg-border/30 p-4 transition-colors duration-300 hover:bg-border/50 md:max-h-[260px] lg:p-6",
              )}
            >
              <div className="flex size-full justify-center">
                ...
                <div className="-translate-x-1/2 absolute bottom-0 left-1/2 w-fit">
                  <div
                    className={cn(
                      "relative aspect-square w-22 flex-center transition-all duration-300 group-hover:grayscale-0 sm:w-35 md:grayscale-60",
                      partner.id === 7 && "w-27 md:w-55",
                    )}
                  >
                    <Image
                      src={partner.src}
